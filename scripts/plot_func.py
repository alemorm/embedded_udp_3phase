...
def live_plotter(x_vec,y1_data,y2_data,y3_data,line1,line2,line3,identifier='',pause_time=0.1):
    if any([line1==[], line2==[], line3==[]]):
        ...
        line1, = ax.plot(x_vec,y1_data,'r-',alpha=0.8)        
        line2, = ax.plot(x_vec,y2_data,'g-',alpha=0.8)        
        line3, = ax.plot(x_vec,y3_data,'b-',alpha=0.8)        
        #update plot label/title
        plt.ylabel('Amplitude')
        plt.title(f'3 Phase Power: {identifier}')
        plt.ylim([-125, 125])
        plt.show()
